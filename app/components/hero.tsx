    ...
    <section id="home" className="relative overflow-hidden pt-36 pb-20 sm:pt-44 sm:pb-28">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-14 px-4 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:gap-10 lg:px-8">
        <div className="animate-fade-up text-center lg:text-left">
          <span className="glass inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-medium tracking-wide text-brand-200">
          ...
          <ul className="mt-8 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm text-zinc-400 lg:justify-start">
        ...
        <div className="animate-fade-up [animation-delay:150ms]">
          <div className="glass-strong relative mx-auto max-w-md rounded-3xl p-5 lg:max-w-none">
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              <div>
                <p className="text-sm font-semibold text-white">Project Overview</p>
                <p className="text-xs text-zinc-500">Live delivery dashboard</p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="glass rounded-2xl p-4">
                ...
                <ChartIcon className="mt-2 h-5 w-5 text-accent-400" />
              </div>
              <div className="glass rounded-2xl p-4">
                <p className="text-xs text-zinc-400">Happy Clients</p>
                <p className="mt-1 text-2xl font-bold text-white">80+</p>
                <SparklesIcon className="mt-2 h-5 w-5 text-violet-400" />
              </div>
            </div>

            <div className="glass mt-3 rounded-2xl p-4">
              <p className="text-xs text-zinc-400">Client growth</p>
            ...
            <div className="mt-3 grid grid-cols-3 gap-3 text-center">
              {[
                ...
                <div key={item.label} className="glass rounded-xl py-3">
                  <p className={`text-sm font-semibold ${item.tint}`}>{item.label}</p>
                  <p className="text-[11px] text-zinc-500">In-house</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="mx-auto mt-20 max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-4 sm:grid-cols-3">
          {FEATURES.map((feature) => (
            <div
              key={feature.title}
              className="glass group flex items-start gap-4 rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1 hover:bg-white/[0.07]"
            >
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500/30 to-violet-500/30 text-brand-300 transition-colors group-hover:text-accent-300">
                <feature.icon className="h-5 w-5" />
              </span>
              <span>
                <span className="block text-sm font-semibold text-white">{feature.title}</span>
                <span className="mt-1 block text-sm text-zinc-400">{feature.text}</span>
              </span>
            </div>
          ))}
        </div>
